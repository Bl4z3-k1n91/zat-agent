...
To introduce a new metric, developers must modify the GrpcPlugin (agent/plugins/grpc_plugin.py).
...
def GetCapabilities(self, request, context):
    sensors = [
        ...
        "/custom/new_metric_path"  # Register new metric
    ]
    return zat_pb2.CapabilitiesResponse(supported_sensors=sensors, ...)
```
...
def Subscribe(self, request, context):
    # ... inside event loop ...
    if request.xpath == "/custom/new_metric_path":
        # Logic to read from file, API, or hardware
        val = my_custom_reading_function()
    
    yield zat_pb2.TelemetryData(..., value=val)
```
...
python -m grpc_tools.protoc -I./proto --python_out=./agent --grpc_python_out=./agent ./proto/zat.proto
python -m grpc_tools.protoc -I./proto --python_out=./controller --grpc_python_out=./controller ./proto/zat.proto
```
